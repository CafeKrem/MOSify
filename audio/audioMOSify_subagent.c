...
char mosc[12]="sofiene";
char moslo[12]="jelassi";
...
/* Our SIGALARM handler which will change our variables */
...
void AlrmHandler(int signo) {
   ...
   filep = fopen("/var/tmp/audioqual.conf", "r");
    if(NULL ==  filep) {
        return;
    }
    ...
    */ 
   nb_val=sscanf(line, "%s %s", mosc, moslo);
   printf("I read %d values\n", nb_val);
   printf("that: %f..%f\n", atof(mosc), atof(moslo));
   
   if(atof(moslo) < 3) send_audioMOSHit_trap(atof(moslo));
...
static void usage(void) {
   printf("usage: audioMOSify [-D<tokens>] [-f] [-L] [-M] [-H] [LISTENING ADDRESSES]\n"
...
main (int argc, char **argv) {
  ...
  while ((ch = getopt(argc, argv, "D:fHLM-:x:")) != EOF)
    ...
    case 'H':
      ...
      init_agent("audioMOSify");        /* register our .conf handlers */
      init_audioMOSify();  
      init_snmp("audioMOSify");
  ...
  init_agent("audioMOSify");

  /* init audioMOSify mib code */
  init_audioMOSify();
  init_audioConfigTable(); 

  /* read audioMOSify.conf files. */
  init_snmp("audioMOSify");
  ...
  /* setup our alaremhandler so we can see value changes */
