...
------------------------------------------------------------------------
  ...
  the audioConfigTable table.  The agent will start by looking for the right
  ...
  file defaults/table-audioConfigTable.m2d, and then re-running mib2c.
    ...
    in the audioConfigTable.h header.
    ...
    will be generated:
      ...
      audioConfigTable_allocate_data
      audioConfigTable_release_data
 ...
 Index(es) for the audioConfigTable table
  ------------------------------------------------------------
  The index(es) for the audioConfigTable table are:

     audioParamConfigName:
...
************************************************************************
audioConfigTable File Overview
------------------------------------------------------------------------
  Several files have been generated to implement the audioConfigTable
  table. We'll go through these files, one by one, explaining each and
  letting you know which you need to edit.


File: audioConfigTable_data_access.[c|h]
------------------------------------------------------------------------
  The audioConfigTable_data_access file contains the interface to your data in
  ...
  FUNC : audioConfigTable_indexes_set
  WHERE: audioConfigTable_data_access.c
...
File: audioConfigTable_data_get.c
------------------------------------------------------------------------
  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : audioParamConfigValue_get



File: audioConfigTable_data_set.c
------------------------------------------------------------------------
...
NOTE: Depending on your configuration, some of the functions used in the
      ...
      audioConfigTable table.

      Conversely, the examples below may not include some functions that
      were generated for the audioConfigTable table.

To watch the flow of the audioConfigTable table, use the
following debug tokens:

        snmp_agent
        helper:table:req
        audioConfigTable
        verbose:audioConfigTable
        internal:audioConfigTable

e.g.
        snmpd -f -Le -DaudioConfigTable,verbose:audioConfigTable,internal:audioConfigTable
...
/*---------------------------------------------------------------------
 * MOSIFY-AUDIO-MIB::audioConfigTableEntry.audioParamConfigName
 * audioParamConfigName is subid 1 of audioConfigTableEntry.
 * Its status is Current, and its access level is ReadOnly.
 * OID: .1.3.6.1.3.79.1.1.1.1.1
 * Description:
A name of an audio parameter
 *
 * Attributes:
